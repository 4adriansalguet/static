
    <section class="___section_3">
        <div class="___section_3_container">
            <h3 class="services_after">WHAT WE DO</h3>
            <h2><span>Our</span> Services</h2>
            <div class="___section_3_flex">

                <div class="__section_3_card __section_3_card_bg_1">
                    <div class="__section_3_card_body">
                        <h2>Interior Painting</h2>
                        <a href="#">LEARN MORE</a>
                    </div>
                </div>

                <div class="__section_3_card __section_3_card_bg_2">
                    <div class="__section_3_card_body">
                        <h2>Interior Painting</h2>
                        <a href="#">LEARN MORE</a>
                    </div>
                </div>

                <div class="__section_3_card __section_3_card_bg_3">
                    <div class="__section_3_card_body">
                        <h2>Interior Painting</h2>
                        <a href="#">LEARN MORE</a>
                    </div>
                </div>

            </div>
        </div>
    </section>

	<section id="section_4">

		<section class="kitchen_section">
        <div class="kitchen-container">
			<img src="./public/images/gallery/kitchen.fw.png" alt="">
			<div class="kitchen-des">
				<h3>WELCOME TO</h3>
				<h4 class="title">GARCIA PAINTING</h4>
				<P class="sub-title">A TERRIFIC INVESTMENT</P>
				<p>
                A new coat of paint adds value and protection to any kind of property, 
                whether that property is commercial, residential or industrial. <br> <br>  
                However, the value of that property will only be enhanced by a good paint job, 
                the kind that’s provided by professionals like Garcia Painting.
                </p>
				<a href="#">LEARN MORE</a>
			</div>
		</div>
        </section>

		<section class="wall_section">
        <div class="wall-container">
			<div class="wall-des">
			<h3 class="title">Extensive Experience</h3>
            <p>
            Established in 2008, we’ve got extensive experience and the elite expertise that comes along with it. 
            Let us be your one and only choice for all sorts of painting projects. <br> <br>
            As a locally owned company, we are more than happy to work with fellow members of the Gilroy community.
            </p>            
				<a class="sub-title" href="#">LEARN MORE</a>
			</div>
			<img src="./public/images/gallery/housefw.png" alt="">
		</div>
        </section>

        <section class="house_yard">
        <div class="kitchen-container">
			<img src="./public/images/gallery/house4fw.png" alt="">
			<div class="kitchen-des">
				<!-- <h3>WELCOME TO</h3> -->
				<h4 class="title gal-col3">Restore and Renovate</h4>
				<!-- <P class="sub-title">A TERRIFIC INVESTMENT</P> -->
				<p>
                At Garcia painting, we work to restore, renovate and create 
                the environment that best suits your lifestyle. <br> <br>
                We are committed to professionalism, creative ideas with color and design,
                 respect for your property and time, and a pleasant process with a job well done.
                </p>
				<a href="#">LEARN MORE</a>
			</div>
		</div>
        </section>
        
	</section>

     <section id="gallery_section">
            <h3>WHAT WE DO</h3>
            <p><span>Our</span> Gallery</p>

            <div class="gallery_container">
            <div class="pictures_left">  
                <img src="./public/images/icons/dining.fw.png" alt="">
                <img src="./public/images/icons/outdoor.fw.png" alt="">
                <div class="pkyu">
                    <img src="./public/images/icons/lounge.fw.png" alt="">
                </div>
            </div>

            <div class="pictures_right">
                <img class="suburbs" src="./public/images/icons/suburbs.fw.png" alt="">
                <div class="strp_subr">
                <img class="stripe" src="./public/images/icons/stripehouse.fw.png" alt="">
                <img  class="sala" src="./public/images/icons/salaset.fw.png  " alt="">
                </div>
            </div>
            
            </div>
            
    </section> 


